...
void desencriptando () {
    ...
    long long int n_value = p * q;
    long long int d = inverso(e, (p - 1) * (q - 1), (p - 1) * (q - 1), e % ((p - 1) * (q - 1)), 0, 1, 1, -e / ((p - 1) * (q - 1)));

    FILE *mensagem = fopen("mensagem.txt", "w");
    if (mensagem == NULL) {
        printf("Erro ao abrir o arquivo para escrever a mensagem desencriptada.\n");
        return;
    }
